...
<fieldlist>
	<group name="group" xpath="/root/group" minoccurs="0" maxoccurs="unbounded"></group>
	...
	<field name="fieldname1" datatype="validationName1" content="type1" xpath="/root/group/element1" title="title1" indexed="true" minoccurs="0" maxoccurs="unbounded">display name1</field>
...
<xsl:template match="/">
	...
	<xsl:if test="count($fields/field) &gt; 0">
		<fieldlist>
			<xsl:copy-of select="$fields/group"/>
...
<xsl:template name="processElement">
	...
	<xsl:variable name="datalistName">
		<xsl:if test="name()='ektdesignns_checklist' or name()='ektdesignns_choices' or name()='select'">
	...
	<field name="{@ektdesignns_name}" datatype="{$datatype}" basetype="{$basetype}" xpath="{$new-xpath}" title="{@title}">
		...
		<xsl:if test="string-length($datalistName) &gt; 0">
			<xsl:attribute name="datalist"><xsl:value-of select="$datalistName"/></xsl:attribute>
		</xsl:if>
		<xsl:choose>
			<xsl:when test="@ektdesignns_translate">
				<xsl:attribute name="translate"><xsl:value-of select="@ektdesignns_translate"/></xsl:attribute>
			</xsl:when>
			<xsl:when test="@ektdesignns_calculate">
				<xsl:attribute name="translate">false</xsl:attribute>
			</xsl:when>
		</xsl:choose>
		<xsl:if test="@ektdesignns_minoccurs != '1'">
			<xsl:attribute name="minoccurs"><xsl:value-of select="@ektdesignns_minoccurs"/></xsl:attribute>
		</xsl:if>
		<xsl:if test="@ektdesignns_maxoccurs != '1'">
			<xsl:attribute name="maxoccurs"><xsl:value-of select="@ektdesignns_maxoccurs"/></xsl:attribute>
		</xsl:if>
		...
		<datalist name="{$datalistName}">
			<xsl:choose>
				<xsl:when test="@ektdesignns_datasrc">
					<xsl:attribute name="ektdesignns_datasrc">
						<xsl:choose>
							<xsl:when test="contains(@ektdesignns_datasrc,'[srcpath]')">
								<xsl:value-of select="concat($srcPath,substring-after(@ektdesignns_datasrc,'[srcpath]'))"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="@ektdesignns_datasrc"/>
							</xsl:otherwise>
						</xsl:choose>
					</xsl:attribute>
					<xsl:copy-of select="@ektdesignns_dataselect|@ektdesignns_captionxpath|@ektdesignns_valuexpath|@ektdesignns_datanamespaces"/>
				</xsl:when>
...
<xsl:template match="ektdesignns_group|fieldset[@ektdesignns_nodetype='element']|table[@ektdesignns_nodetype='element']|tr[@ektdesignns_nodetype='element']" priority="1">
	...
	<group name="{@ektdesignns_name}" xpath="{$new-xpath}">
		<xsl:if test="@ektdesignns_minoccurs != '1'">
			<xsl:attribute name="minoccurs"><xsl:value-of select="@ektdesignns_minoccurs"/></xsl:attribute>
		</xsl:if>
		<xsl:if test="@ektdesignns_maxoccurs != '1'">
		...
		<xsl:choose>
			<xsl:when test="legend/node()">
				<xsl:copy-of select="legend/node()"/>
			</xsl:when>
			<xsl:when test="caption/node()">
...
<xsl:template name="getDatatype">
	<xsl:param name="knowntype"/>
	<xsl:choose>
		<xsl:when test="name()='ektdesignns_checklist' or (name()='select' and @multiple)">
			<xsl:value-of select="'selection'"/> <!-- used to distinguish ektdesignns_mergelist and ektdesignns_mergefield -->
		</xsl:when>
		<xsl:when test="contains(@ektdesignns_datatype,':')">
			<xsl:value-of select="substring-after(@ektdesignns_datatype,':')"/>
		</xsl:when>
...
<xsl:template name="getBasetype">
	...
	<xsl:choose>
		...
		<xsl:when test="name()='ektdesignns_resource' and $datatype='long'">
