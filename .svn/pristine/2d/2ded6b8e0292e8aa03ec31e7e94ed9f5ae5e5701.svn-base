...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="Item[ItemType!='Submenu']">
		<xsl:choose>
			<xsl:when test="ItemId='0' and ItemType='Javascript' and ItemSubType='0' and ItemLevel='1'">
			</xsl:when>
			<xsl:otherwise>
				<li>
					...
					<a>
						...
						<xsl:if test="ItemId='0' and ItemType='Javascript' and ItemSubType='0'">
							<img>
								<xsl:attribute name="border">0</xsl:attribute>
								<xsl:choose>
									<xsl:when test="contains(ItemLink,'action=AddMenuItem')">
										<xsl:attribute name="src"><xsl:value-of select="/MenuDataResult/Info/SitePath"/>css/images/fmadd2.png</xsl:attribute>
										<!--<xsl:attribute name="src"><xsl:value-of select="/MenuDataResult/Info/AppPath"/>/images/application/fmadd2.png</xsl:attribute>-->
									</xsl:when>
									<xsl:otherwise>
										<xsl:attribute name="src"><xsl:value-of select="/MenuDataResult/Info/SitePath"/>css/images/fmedit.png</xsl:attribute>
									</xsl:otherwise>
								</xsl:choose>
							</img>
						</xsl:if>
	...
	<xsl:template name="makeButton">
		<xsl:if test="not((/MenuDataResult/Info/MenuFragment='true') and (MenuLevel = '1') and $slaveMenu='true')">
			...
			<xsl:if test="MenuLevel!='1'">
				<img style="margin-top: 1px; margin-left: 1px;" alt="Access sub menu">
					<xsl:attribute name="border">0</xsl:attribute>
					<xsl:attribute name="src"><xsl:value-of select="/MenuDataResult/Info/SitePath"/>css/images/fmsubarrow.png</xsl:attribute>
				</img>
			</xsl:if>
		</xsl:if>
		<input type="hidden">
